...
def home():
    ...
    if (not str(page).isnumeric()):
        page = 1
    page = int(page)
    posts = posts[(page-1)*int(params["no_of_posts"]):(page-1)*params["no_of_posts"] + params["no_of_posts"]]    
    if (page==1):
        prev = "#"
...
def edit(sno):
    if ('user' in session) and (session['user'] == params['Admin_user']):
        if request.method == "POST":
            ...
            if sno == '0':
                post = Posts(Title = title, slug = slug, Content = content, Date = Date, img_file = img_file, tagline = tline) 
                db.session.add(post)
                db.session.commit()  
